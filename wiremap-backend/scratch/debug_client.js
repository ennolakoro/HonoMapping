...
async function run() {
  ...
  if (row.parameter_list_json) {
    const list = JSON.parse(row.parameter_list_json as string);
    console.log(`Total parameter: ${list.length}`);
    console.log("WLAN/SSID/Pass di list:");
    console.log(list.filter((p: string) => p.toLowerCase().includes('wlan') || p.toLowerCase().includes('wifi') || p.toLowerCase().includes('ssid') || p.toLowerCase().includes('key') || p.toLowerCase().includes('passphrase')));
  ...
  if (row.raw_modem_params_json) {
    const raw = JSON.parse(row.raw_modem_params_json as string);
